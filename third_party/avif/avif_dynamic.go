...
// applyColorMatrix 把色彩描述写入 avifImage；matrix 非零时 RGB→YUV 使用该矩阵，
// primaries/transfer 为零时按 matrix 配套补全。
func applyColorMatrix(img *avifImage, matrix, primaries, transfer uint16) {
	if matrix == 0 {
		return
	}
	img.MatrixCoefficients = matrix
	if primaries != 0 {
		img.ColorPrimaries = primaries
	} else if matrix == avifMatrixCoefficientsBT2020nc {
		img.ColorPrimaries = 9 // BT.2020
	}
	if transfer != 0 {
		img.TransferCharacteristics = transfer
	} else if matrix == avifMatrixCoefficientsBT2020nc {
		img.TransferCharacteristics = 18 // ARIB STD-B67 (HLG)
	}
}
...
func encodeDynamic(w io.Writer, m image.Image, quality, qualityAlpha, speed int, subsampleRatio image.YCbCrSubsampleRatio, lossless bool, matrix, primaries, transfer uint16) error {
	...
	applyColorMatrix(img, matrix, primaries, transfer)
